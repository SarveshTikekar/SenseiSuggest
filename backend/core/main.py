...
from .schemas import *
from .utils import argon2_pwd_hasher, generate_uuid, password_verifier
...
async def get_anime_stats(user_id: int, supabase: AsyncClient = Depends(get_supabase)):
    ...
    res_watched = (
        await supabase.table("user_watched_anime")
        .select("animeId", count="exact")
        .eq("userId", user_id)
        .execute()
    )
    res_bookmarked = (
        await supabase.table("user_bookmarked_anime")
        .select("animeId", count="exact")
        .eq("userId", user_id)
        .execute()
    )
    res_watching = (
        await supabase.table("user_watching_anime")
        .select("animeId", count="exact")
        .eq("userId", user_id)
        .execute()
    )
...
async def sort_results(
    sort_param: str, sort_order: str, supabase: AsyncClient = Depends(get_supabase)
):

    response = (
        await supabase.table("anime")
        .select("*")
        .order(f"{sort_param}", desc=True if sort_order == "desc" else False)
        .execute()
    )
...
async def get_all_anime(supabase: AsyncClient = Depends(get_supabase)):
    ...
    if not results:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"No Anime found"
        )
    list_of_animes = []
...
async def get_anime_info(
    anime_name: str, supabase: AsyncClient = Depends(get_supabase)
):

    response = (
        await supabase.table("anime")
        .select("*, anime_genres(genreId, genres(name)), seasons(*)")
        .eq("animeName", anime_name)
        .execute()
    )
    data = response.data
    ...
    if not data:
        raise HTTPException(
            ...
            detail=f"Anime {anime_name} not found",
        )
    ...
    genre_list = [
        g["genres"]["name"]
        for g in query_result.get("anime_genres", [])
        if g.get("genres")
    ]
    ...
    response = (
        ...
        .select("*")
        .eq("animeId", query_result.get("animeId"))
        .order("seasonNumber")
        .execute()
    )
    seasons_info = response.data

    # Fetch ratings data of the anime
    ratingsResponse = (
        await supabase.table("ratings")
        .select("anime(animeName,image_url_base_anime), score, review_text")
        .eq("animeId", query_result.get("animeId"))
        .execute()
    )
    ...
    ratingsData = [
        RatingsRetrieved(
            ...
            animeName=data.get("anime", {}).get("animeName", ""),
            score=data.get("score"),
            review_text=data.get("review_text"),
            image_url_base_anime=data.get("anime", {}).get("image_url_base_anime", "")
        )

        for data in (ratingsResponse.data or []) if data is not None
    ]
   
    avg_rating = sum(r.score for r in ratingsData) / len(ratingsData) if ratingsData else None
...
async def get_user_info(
    user: UserBasicCreate, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Due to error: {e} caused hence the new user was not created hence please try again",
        )

    return {
        "message": f"User {user.userName} created successfully and welcome to this Anime Recommendation system"
    }
...
async def login(credentials: userLogin, supabase: AsyncClient = Depends(get_supabase)):
    ...
    response = (
        ...
        .select("*")
        .or_(
            f"userName.eq.{credentials.userName_or_email},email.eq.{credentials.userName_or_email}"
        )
        .execute()
    )
...
async def recommendations(user_id: int, supabase: AsyncClient = Depends(get_supabase)):
    ...
    if genre_resp.data:
        for item in genre_resp.data:
            if item.get("genres"):
                ...
                genre_anime_distribution[name] = (
                    genre_anime_distribution.get(name, 0) + 1
                )

    genre_anime_distributionList = [
        {name: count} for name, count in genre_anime_distribution.items()
    ]

    # most popular animes
    rt_resp = (
        await supabase.table("ratings")
        .select("score, anime(animeId, animeName, releaseDate, image_url_base_anime)")
        .execute()
    )
    anime_stats = {}
    if rt_resp.data:
    ...
    user_resp = (
        await supabase.table("users")
        .select("locationId, locations(state, country)")
        .eq("userId", user_id)
        .execute()
    )
    ...
    titles = {
        "primary": "Trending in " + user_location
        if is_cold_start
...
async def get_states(countryName: str, supabase: AsyncClient = Depends(get_supabase)):
    ...
    response = (
        await supabase.table("distinct_country_state_pairs")
        .select("state")
...
async def add_to_watched_list(
    animeListUpdate: AnimeListUpdate, supabase: AsyncClient = Depends(get_supabase)
):

    check_watched = (
        await supabase.table("user_watched_anime")
        .select("*")
        .eq("userId", animeListUpdate.userId)
        .eq("animeId", animeListUpdate.animeId)
        .execute()
    )
    if check_watched.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime already in watched list",
        )

    await (
        supabase.table("user_watching_anime")
        .delete()
        .eq("userId", animeListUpdate.userId)
        .eq("animeId", animeListUpdate.animeId)
        .execute()
    ...
    try:
        await (
            supabase.table("user_watched_anime")
            .insert(
                {"userId": animeListUpdate.userId, "animeId": animeListUpdate.animeId}
            )
            .execute()
        )
    except Exception as e:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be added to watchedlist",
        )
...
async def add_to_watching_list(
    ...
    if check_watching.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime already in watching list",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be added to watching list",
        )
...
async def add_as_bookmarked(
    bookmarkData: BookmarkAnime, supabase: AsyncClient = Depends(get_supabase)
):
    """Check if anime is already bookmarked by the user"""
    check_if_bookmarked = (
        await supabase.table("user_bookmarked_anime")
        .select("*")
        .eq("userId", bookmarkData.userId)
        .eq("animeId", bookmarkData.animeId)
        .execute()
    )

    if check_if_bookmarked.data:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT, detail="Anime already bookmarked"
        )

    """ If not bookmarked then add it as a bookmark """
    try:
        await (
            supabase.table("user_bookmarked_anime")
            .insert({"userId": bookmarkData.userId, "animeId": bookmarkData.animeId})
            .execute()
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be bookmarked due to error: {e}",
        )
...
async def add_to_scrapbook(
    ...
    # 6-image limit check
    existing_count = (
        await supabase.table("anime_scrapbook")
    ...
    public_url = await supabase.storage.from_("anime_scrapbook").get_public_url(
        storage_path
    )
    ...
    try:
        ...
        await (
            supabase.table("anime_scrapbook")
            .insert(scrapbook_entry.model_dump(mode="json"))
            .execute()
        )
    except Exception as e:
...
async def remove_from_scrapbook(
    ...
    if not check_entry.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Scrapbook entry not found"
        )
    ...
    try:
        await supabase.storage.from_("anime_scrapbook").remove([storage_path])
        await (
            supabase.table("anime_scrapbook").delete().eq("id", scrapbook_id).execute()
        )
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error during removal: {e}"
        )
...
async def get_scrapbook(user_id: int, supabase: AsyncClient = Depends(get_supabase)):
    ...
    try:
        response = (
            await supabase.table("anime_scrapbook")
            .select(
                "id, userId, animeId, anime(animeName), screenshotUrl, screenshotDescription, created_at"
            )
            .eq("userId", user_id)
            .execute()
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Error fetching scrapbook: {e}",
        )

    return {
        "message": f"Scrapbook entries fetched successfully for user {user_id}",
        "data": response.data,
    }


@app.patch("/remove_as_bookmarked", status_code=status.HTTP_200_OK)
async def remove_as_bookmarked(
    bookmarkData: BookmarkAnime, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    if not check_if_bookmarked.data:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Anime not bookmarked yet"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be removed from bookmarks due to error: {e}",
        )
...
async def remove_from_watched_list(
    ...
    if not check_watched.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Anime not in watched list"
        )
    ...
    try:
        ...
        print(
            f"Error removing from watched list for user {animeListUpdate.userId}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be removed from watched list",
        )
...
async def remove_from_watching_list(
    animeListUpdate: AnimeListUpdate, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    if not check_watching.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime not in watching list",
        )
    ...
    try:
        ...
        print(
            f"Error removing from watching list for user {animeListUpdate.userId}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Anime couldn't be removed from watching list",
        )
...
async def user_profile(user_id: int, supabase: AsyncClient = Depends(get_supabase)):
    ...
    response = (
        await supabase.table("users")
        .select(
            "*, user_watched_anime(animeId, anime(*)), user_watching_anime(animeId, anime(*)), user_bookmarked_anime(animeId, anime(*))"
        )
        .eq("userId", user_id)
        .execute()
    )
    ...
    watchedList = [
        ...
        for item in result.get("user_watched_anime", [])
        if item.get("anime")
    ]
    ...
    watchingList = [
        ...
        for item in result.get("user_watching_anime", [])
        if item.get("anime")
    ]
    ...
    bookmarkedList = [
        ...
        for item in result.get("user_bookmarked_anime", [])
        if item.get("anime")
    ]
...
async def initiate_friend_request(
    requestData: FriendRequest, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    resp = (
        await supabase.table("users")
        .select("friends")
        .eq("userId", requestData.sender_id)
        .execute()
    )
    if not resp.data:
        raise HTTPException(
    ...
    pending_check = (
        await supabase.table("friend_requests")
        .select("*")
...
async def friend_request_process(
    requestData: FriendRequestProcess, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    resp = (
        ...
        .eq("receiver_id", requestData.receiver_id)
        .eq("status", "PENDING")
        .order("req_created_at", desc=True)
        .limit(1)
        .execute()
    )
    ...
    if requestData.action == "ACCEPT":
        ...
        await (
            supabase.table("users")
            .update({"friends": list(sender_friends)})
            .eq("userId", requestData.sender_id)
            .execute()
        )
        ...
        receiver_resp = (
            await supabase.table("users")
            .select("friends")
            .eq("userId", requestData.receiver_id)
            .single()
            .execute()
        )
        ...
        await (
            supabase.table("users")
            .update({"friends": list(receiver_friends)})
            .eq("userId", requestData.receiver_id)
            .execute()
        )
...
async def find_allies(
    userId: int, query: str, supabase: AsyncClient = Depends(get_supabase)
):
...
async def get_pending_requests(
    userId: int, supabase: AsyncClient = Depends(get_supabase)
):
    ...
    resp = (
        await supabase.table("friend_requests")
        .select(
            "*, sender:users!friend_requests_sender_id_fkey(userName, profilePicture)"
        )
...
async def search_anime(query: str, supabase: AsyncClient = Depends(get_supabase)):
    ...
    response = (
        await supabase.table("lowercased_anime_names")
        .select("*")
        .ilike("animeName", f"{query}%")
        .execute()
    )
...
async def rate_anime(
    ratingData: RatingDataFromUser, supabase: AsyncClient = Depends(get_supabase)
):
    """Either create a new record or update the existing one for the user-anime pair"""
    now = datetime.now(timezone.utc)
    rating_payload = AddRating(
        **ratingData.model_dump(), created_at=now, updated_at=now, ratingId=uuid.uuid4()
    )

    resp = (
        await supabase.table("ratings")
        .insert(rating_payload.model_dump(mode="json"))
        .execute()
    )
...
@app.patch("/update_anime_rating", status_code=HTTP_200_OK)
async def update_anime_rating(
    ratingData: RatingDataFromUser, supabase: AsyncClient = Depends(get_supabase)
):
    """We update the anime rating record for the user"""

    """ First get the particular resource from Supabase """
    try:
        rating_payload = (
            await supabase.table("ratings")
            .update(
                {**ratingData.model_dump(), "updated_at": datetime.now(timezone.utc)}
            )
            .eq("userId", ratingData.userId)
            .eq("animeId", ratingData.animeId)
            .execute()
        )

        if rating_payload:
            print("Rating Updated")
            print(rating_payload.data)
